...
export async function getAuditionData(): Promise<AuditionData> {
  ...
  try {
    ...
    raw = (await payload.findGlobal({ slug: 'audition', depth: 2 })) as Record<string, unknown>
  } catch {
  ...
  type RawAudition = {
    ...
    flow?: Partial<Omit<AuditionData['flow'], 'steps'>> & {
      steps?: AuditionData['flow']['steps']
    }
    ...
    faq?: Partial<Omit<AuditionData['faq'], 'items'>> & {
      items?: AuditionData['faq']['items']
    }
    cta?: Partial<AuditionData['cta']>
  }
  const r = raw as unknown as RawAudition
